...
**Load profiles** activates existing MagguuUI profiles onto this character. It does not re-import Magguu profiles. Class layouts are installed per character. Spec changes use layouts named \`Magguu - Class Spec\`.`,
    sortOrder: 2,
  },
  {
    category: 'installation',
...
**Individual profiles** — import only the piece you want from **Setup**. Required: EllesmereUI and class layouts. Optional Setup buttons: BigWigs, Northern Sky, EXBoss, Smart Reminders. WIM, Waypoint UI, HandyNotes, Talent Tree Tweaks, GTFO, BugSack, and Premade Groups Filter apply with Apply Magguu profiles and Load profiles.

**Magguu Settings** applies MagguuUI overlay/QoL only (fonts, class colors, Giant options panel, Skinning NAMES & COLORS toggles, MagguuUI chat channels and size with Services off General, MagguuUI extras, fixed tooltip). It does **not** re-import Magguu profiles. **Magguu Settings** and **Load profiles** sit side by side below Apply Magguu profiles. Scale \`0.58\` comes with Apply Magguu profiles / Magguu Settings; Magguu Settings does not re-import Ellesmere — there is no Set-scale-only button. **Load profiles** activates MagguuUI profiles and does not re-import them (class layouts still install per character).

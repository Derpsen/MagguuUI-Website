export const DEFAULT_HOME_CONTENT = [
  ...
  { page: 'home', section: 'hero', key: 'description', value: 'MagguuUI is a native <strong>EllesmereUI</strong> module for WoW Retail. Install <strong>EllesmereUI</strong> and the four MagguuUI addon folders, then open <code>/mui</code> and run <strong>Install All</strong>. BigWigs and Northern Sky stay optional.', type: 'html', sortOrder: 3, locale: 'en' },
  ...
  { page: 'home', section: 'features', key: 'feature_3_text', value: 'Skinning is one <strong>NAMES &amp; COLORS</strong> section (two columns): split unit-frame names, split party and raid names, and class-colored keybind modifiers (AuraBuff counts centered). QoL covers Death Release, Co-Tank, Stealth/Stance, Spell Alerts, party and raid item level (and 2P/4P), <strong>Smart Tab</strong>, Hide Services on General (secret-safe), <strong>Quick Focus</strong>, an <strong>Audio device switcher</strong> on the Ellesmere speaker, and <strong>Targeted Spell Bars</strong> via Ellesmere (leave EXBoss MythicCast off).', type: 'html', sortOrder: 7, locale: 'en' },
  ...
  { page: 'home', section: 'features', key: 'feature_3_text', value: 'Skinning ist eine Kategorie <strong>NAMEN &amp; FARBEN</strong> (zwei Spalten): geteilte Unit-Frame-Namen, Party-/Raid-Namen und klassenfarbige Keybind-Modifier (AuraBuff-Zähler zentriert). QoL: Death Release, Co-Tank, Stealth/Stance, Spell Alerts, Itemlevel in Party und Raid (2P/4P), <strong>Smart Tab</strong>, Hide Services auf General (secret-safe), <strong>Quick Focus</strong>, ein <strong>Audio-Geräte-Umschalter</strong> am Ellesmere-Lautsprecher und <strong>Targeted Spell Bars</strong> über Ellesmere (EXBoss MythicCast aus).', type: 'html', sortOrder: 7, locale: 'de' },
...
export const DEFAULT_GUIDE_CONTENT = [
  ...
  { page: 'guide', section: 'intro', key: 'text', value: 'Install EllesmereUI 9.0.6+ (live 9.1.6), add the four MagguuUI addon folders, then open /mui and run Install All. Magguu Settings (overlay/QoL only), Load profiles (activate only), and WowUp copy popups live on Setup. BigWigs, LittleWigs, and Northern Sky Raid Tools are optional.', locale: 'en', type: 'text', sortOrder: 1 },

  { page: 'guide', section: 'steps', key: 'step_1_title', value: '1. Install EllesmereUI', locale: 'en', type: 'text', sortOrder: 10 },
  { page: 'guide', section: 'steps', key: 'step_1', value: 'MagguuUI is a native EllesmereUI module. Install **EllesmereUI 9.0.6 or newer** first from CurseForge, Wago, or WoWInterface, and leave it enabled.\n\nWithout EllesmereUI, MagguuUI will not load.\n\nEllesmere\'s start popup is skipped. MagguuUI Setup opens on that login so you can run Install All.', locale: 'en', type: 'markdown', sortOrder: 11 },
  ...
  { page: 'guide', section: 'steps', key: 'step_3', value: 'Only EllesmereUI is required. Install extra addons only if you want MagguuUI to configure them:\n\n- **BigWigs** — encounter timers, MagguuUI bar style, and bundled callout sounds\n- **LittleWigs** — dungeon timers for BigWigs (WowUp starter pack; no separate Magguu profile)\n- **Northern Sky Raid Tools** — raid notes and alerts\n- **WIM**, **Waypoint UI**, and **EXBoss** — optional Magguu imports when those addons are present\n- **EXBoss** — MagguuUI appearance and MagguuUI role configs; import again replaces those MagguuUI configs\n- **EXCore** — in the WowUp starter pack with EXBoss; no separate Magguu profile\n\nMissing addons are skipped.\n\nCopy the **WowUp starter pack** and **optional pack** from Magguu **Setup** (`Ctrl+C`, then paste in WowUp). The Strings page is a backup of the same strings. MagguuUI shows a copy popup; it does not install addons itself.\n\nStarter pack: EllesmereUI, MagguuUI, BigWigs, LittleWigs, Northern Sky, EXBoss, EXCore. Optional pack: BugGrabber, BugSack, HandyNotes, MDT, Raider.IO, Simulationcraft, Talent Tree Tweaks, WIM, Ellesmere WIM Skin, Waypoint UI, GTFO, Premade Groups Filter, Auctionator.\n\nWowUp copy buttons turn green when every addon in that pack is installed.', locale: 'en', type: 'markdown', sortOrder: 31 },

  { page: 'guide', section: 'steps', key: 'step_4_title', value: '4. Open Setup and Install All', locale: 'en', type: 'text', sortOrder: 40 },
  { page: 'guide', section: 'steps', key: 'step_4', value: 'Log in, then open MagguuUI:\n\n- Type `/mui` or `/mui setup` in chat\n- Or open EllesmereUI and choose **MagguuUI → Optionen → Setup**\n\n**Install All** is the gold button for a fresh install. It imports the delta-only Ellesmere bake (keeps MagguuUI overlay/fonts/media/scales) at UI scale `0.58`, applies **Magguu Look**, then companions (BigWigs, Northern Sky, WIM, Waypoint UI, EXBoss) if present, then Cooldown Viewer class layouts. It also writes the HUD Edit Mode layout **MagguuUI** once. Ellesmere MagguuUI import pre-checks **Window & Tooltip Skins**. Wait until you are out of combat.\n\nOn the same tab, **Magguu Settings** and **Load profiles** sit side by side below Install All:\n\n- **Magguu Settings** — MagguuUI overlay and QoL only (fonts, class colors, Giant options panel, Skinning NAMES & COLORS toggles, MagguuUI chat channels and size with Services off General, MagguuUI extras, fixed tooltip). It does **not** reimport the Ellesmere bake\n- **Load profiles** — activate MagguuUI profiles already on the account for this character. It does **not** reimport the bake (class layouts still install per character)\n- **Required / optional MagguuUI imports** — EllesmereUI and class layouts; BigWigs, Northern Sky, WIM, Waypoint UI, and EXBoss\n- **WowUp** — copy the starter pack or optional pack, then paste in WowUp\n\nImport buttons: green means the MagguuUI profile is loaded on this character, orange means installed but not loaded here, red means missing. WowUp buttons turn green when every addon in that pack is installed.\n\nLogging in on an alt asks whether to load those profiles onto that character.', locale: 'en', type: 'markdown', sortOrder: 41 },

  { page: 'guide', section: 'steps', key: 'step_5_title', value: '5. Skinning and QoL', locale: 'en', type: 'text', sortOrder: 50 },
  { page: 'guide', section: 'steps', key: 'step_5', value: 'Stay in **MagguuUI → Optionen**:\n\n- **Skinning** — one **NAMES & COLORS** section (two columns): split unit-frame names, split party and raid names, and class-colored keybind modifiers. Magguu Settings turns these on; reset turns them off\n- **QoL** — death-release protection, co-tank frame and debuffs, stealth and stance reminders, spell-alert opacity, per-spec proc-overlay hiding, party and raid item level (and 2P/4P), Smart Tab (chat channel cycling), Hide Services on General (secret-safe), Quick Focus (modifier-click to focus), an audio device switcher on the Ellesmere speaker icon, and Targeted Spell Bars via Ellesmere Mythic+ Nearby Cast (leave EXBoss MythicCast off)\n\nThese settings live in the MagguuUI Ellesmere profile. MagguuUI_EUI keeps Boiling Point; TopBar, Hearth-Picker, and MagguuUI FPS/MS are not part of this build.', locale: 'en', type: 'markdown', sortOrder: 51 },
...
Install EllesmereUI plus the four MagguuUI addon folders, open \`/mui\`, and run **Install All**.`,
    sortOrder: 0,
  },
  {
    category: 'general',
...
If a MagguuUI profile from a previous install is found, the import will ask before overwriting it.`,
  ...
  },
  {
    ...
    answer: `MagguuUI targets **WoW Retail 12.1** and still loads on Midnight 12.0.

It needs **EllesmereUI 9.0.6 or newer**. Classic and other flavors are not supported.`,
    sortOrder: 4,
  },
...
4. Log in, type \`/mui\`, and run **Install All** on Setup (bake + Magguu Look + companions). Use **Magguu Settings** (overlay/QoL only) and **Load profiles** (activate only) side by side, and copy WowUp packs from the same tab

Ellesmere's start popup is skipped. MagguuUI Setup opens so you can run Install All.

Do not delete MagguuUI_Data / EUI / Media — they are required siblings now.`,
    sortOrder: 0,
  },
  {
    category: 'installation',
...
There is no minimap button and no separate Magguu installer window.`,
  ...
  },
  {
    ...
    answer: `Ellesmere profiles are account-wide, so the MagguuUI profile is already there after the first Install All.

On a new character MagguuUI asks whether to **load those profiles** onto the alt. You can also press **Load profiles** on Setup.

**Load profiles** activates existing MagguuUI profiles onto this character. It does not reimport the bake. Class layouts are installed per character. Spec changes use layouts named \`Magguu - Class Spec\`.`,
    sortOrder: 2,
  },
  {
    category: 'installation',
    question: 'Can I use MagguuUI without EllesmereUI?',
    answer: `**No.** MagguuUI lives inside EllesmereUI.

It will not load unless EllesmereUI is installed **and enabled**.`,
    sortOrder: 3,
  },

  {
    category: 'addons',
    question: 'What does Install All configure?',
    answer: `In order:
...
- **Northern Sky Raid Tools**, **WIM**, **Waypoint UI**, and **EXBoss** (MagguuUI appearance and MagguuUI role configs; re-import replaces MagguuUI configs), if those addons are present
- **Cooldown Viewer** class layouts, then a reload confirm popup

Per-addon MagguuUI imports live on **Setup**. Gold **Install All** (bake + Magguu Look + companions), then **Magguu Settings** (overlay/QoL only) and **Load profiles** (activate only) side by side, plus WowUp copy popups.`,
    sortOrder: 0,
  },
  {
    category: 'addons',
...
- MagguuUI now ships as a BigWigs-style group: keep MagguuUI, MagguuUI [Data], MagguuUI [EUI], and MagguuUI [Media] enabled as sibling folders

Do **not** delete \`MagguuUI_Data\`, \`MagguuUI_EUI\`, or \`MagguuUI_Media\` — they are required. Nested-only copies inside MagguuUI are obsolete.`,
    sortOrder: 2,
  },
  {
    category: 'addons',
    question: 'What is the difference between Install All and individual profiles?',
    answer: `**Install All** — delta-only Ellesmere bake at scale 0.58 plus Magguu Look, then BigWigs / Northern Sky / WIM / Waypoint UI / EXBoss if present, then class layouts.
...
**Magguu Settings** applies MagguuUI overlay/QoL only (fonts, class colors, Giant options panel, Skinning NAMES & COLORS toggles, MagguuUI chat channels and size with Services off General, MagguuUI extras, fixed tooltip). It does **not** reimport the Ellesmere bake. **Magguu Settings** and **Load profiles** sit side by side below Install All. Scale \`0.58\` comes with Install All / Magguu Look; Magguu Settings does not reimport Ellesmere — there is no Set-scale-only button. **Load profiles** activates MagguuUI profiles and does not reimport the bake (class layouts still install per character).
...
On a new character MagguuUI asks whether to load those profiles onto the alt.`,
  ...
  },
  {
    ...
    answer: `No. MagguuUI only needs EllesmereUI plus the four MagguuUI addon folders.
...
- Party and raid item level (and 2P/4P)
- Smart Tab (chat channel cycling)
- Hide Services on General (secret-safe)
- Quick Focus (modifier-click to focus)
- Audio device switcher on the Ellesmere speaker icon
- Targeted Spell Bars via Ellesmere Mythic+ Nearby Cast (leave EXBoss MythicCast off)

Settings are stored in the MagguuUI Ellesmere profile. **Magguu Settings** also turns on MagguuUI extras such as quick loot, skip cinematics, hide talking head, and auto repair — overlay/QoL only, no Ellesmere reimport. MagguuUI_EUI keeps Boiling Point; TopBar, Hearth-Picker, and MagguuUI FPS/MS are not shipped. Tooltips stay at the MagguuUI exported position and do not follow the mouse.`,
    sortOrder: 7,
  },
  {
    category: 'addons',
    question: 'Why does MagguuUI show four AddOns folders?',
    answer: `MagguuUI ships as a BigWigs-style group of **four sibling addons** in \`Interface/AddOns\`:

- MagguuUI
- MagguuUI_Data (title MagguuUI [Data], LoadOnDemand)
- MagguuUI_EUI ([EUI])
- MagguuUI_Media ([Media])

Keep all four enabled. Nested-only Data/EUI/Media inside MagguuUI is obsolete. Do **not** delete the sibling folders — they are required.`,
    sortOrder: 8,
  },
  {
    category: 'addons',
...
Older installer commands are gone.`,
  ...
  },
  {
    ...
    answer: `**Skinning → NAMES & COLORS** (two columns) splits player names: first half white, second half class or reaction color.

- **Split unit-frame names**
- **Split party and raid names**
- **Class-colored keybind modifiers**

Protected or hidden names stay unsplit and white. Magguu Settings turns these on (AuraBuff counts centered). Reset turns them off. Turn the toggles off on the Skinning tab if you prefer a single color.`,
    sortOrder: 10,
  },
  {
    category: 'addons',
    question: 'Why is the UI scale 0.58?',
    answer: `That's MagguuUI's current **4K Ellesmere profile**. **Install All** and **Magguu Settings** apply \`0.58\` after Ellesmere commits the import. There is no Set-scale-only button.

You can still change scale later in EllesmereUI; MagguuUI will not keep rewriting it unless you run Install All again.`,
    sortOrder: 11,
  },
...
1. **EllesmereUI 9.0.6+ is installed and enabled**
2. **All four MagguuUI folders are enabled** — MagguuUI, MagguuUI_Data, MagguuUI_EUI, MagguuUI_Media
3. **You are out of combat**
4. **Ellesmere import popups** — confirm those. MagguuUI accepts the BigWigs MagguuUI popup for you

Re-open Setup with \`/mui\` and run **Install All** again.`,
    sortOrder: 0,
  },
  {
    category: 'troubleshooting',
...
Confirm EllesmereUI and all four MagguuUI folders are enabled, EllesmereUI is 9.0.6 or newer, then \`/reload\`. Open with \`/mui\` rather than looking for a minimap button or a MagguuUI options category in Blizzard's AddOns menu.`,
    sortOrder: 1,
  },
  {
    category: 'troubleshooting',
    question: 'How do I reset MagguuUI and start fresh?',
    answer: `Keep MagguuUI, MagguuUI_Data, MagguuUI_EUI, and MagguuUI_Media enabled, and re-run **Install All** from \`/mui\` → Setup. Nested-only copies inside MagguuUI are obsolete; use the four sibling folders.

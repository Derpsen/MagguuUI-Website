...
MagguuUI ships four sibling addons. GitHub profile sync reads lua from `MagguuUI_Data/AddOns/` and class layouts from `MagguuUI_Data/Classes/`; `MagguuUI.toc` and `CHANGELOG.md` stay at the repository root. Published releases refresh the public changelog as a fallback when a push event was missed.

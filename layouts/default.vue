  ...
  <div class="public-shell min-h-screen flex flex-col transition-colors duration-300"
    ...
    <header class="sticky top-0 z-50 px-3 sm:px-5 pt-3 sm:pt-4">
      <div class="max-w-7xl mx-auto">
        <div class="public-nav-shell rounded-[1.6rem] px-3 sm:px-4 lg:px-5 py-3">
          <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
              <NuxtLink to="/" class="flex items-center gap-3 min-w-0 group">
                ...
                <span class="min-w-0">
                  <span class="block text-[11px] font-semibold uppercase tracking-[0.22em]" :class="isDark ? 'text-silver-500' : 'text-gray-500'">MagguuUI</span>
                  <span class="block text-sm sm:text-base font-semibold truncate" :class="isDark ? 'text-white' : 'text-gray-900'">Curated WoW setup, ready fast</span>
                </span>
              </NuxtLink>

              <span class="hidden lg:inline-flex public-status-chip" :class="siteStatusClasses">
            ...
            <div class="flex items-center gap-2">
              ...
              <NuxtLink v-if="isLoggedIn" to="/admin"
                class="hidden sm:inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition-all"
              ...
              <NuxtLink :to="primaryAction.to"
                class="hidden lg:inline-flex items-center gap-2 btn-gradient rounded-full px-5 py-2.5 text-sm font-semibold text-white">
        ...
        >
          <div v-if="mobileOpen" class="mt-3 public-nav-shell rounded-[1.6rem] p-4 md:hidden">
            <div class="flex items-center justify-between gap-3 mb-4">
              <span class="public-status-chip" :class="siteStatusClasses">
                <span class="inline-block w-2 h-2 rounded-full" :class="siteStatusDot" />
                {{ siteStatusLabel }}
              </span>
              <NuxtLink :to="primaryAction.to"
                class="inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-semibold text-white btn-gradient"
            ...
            <div class="grid gap-2">
              <NuxtLink v-for="link in navLinks" :key="link.to" :to="link.to"
                class="public-quick-card flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium"
                :class="isActive(link.to)
                  ? isDark
    ...
    <div class="relative z-20">
      ...
      <div v-if="breadcrumbLabel" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6">
        <nav class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs sm:text-sm"
          ...
          <UIcon name="i-heroicons-chevron-right" class="w-3.5 h-3.5 opacity-60" />
          ...
          <span v-else class="font-medium" :class="isDark ? 'text-white' : 'text-gray-900'">{{ breadcrumbLabel }}</span>
          <template v-if="breadcrumbSub">
            <UIcon name="i-heroicons-chevron-right" class="w-3.5 h-3.5 opacity-60" />
            <span class="font-medium" :class="isDark ? 'text-white' : 'text-gray-900'">{{ breadcrumbSub }}</span>
          </template>
        </nav>
      </div>
    </div>
    ...
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 translate-y-4" enter-to-class="opacity-100 translate-y-0"
      ...
      <button v-if="showBackToTop" class="fixed bottom-6 right-6 z-40 inline-flex items-center justify-center w-12 h-12 rounded-full text-white btn-gradient shadow-xl" @click="scrollToTop">
...
const primaryAction = computed(() => {
  if (route.path === '/guide') return { to: '/strings', label: 'Browse Strings', icon: 'i-heroicons-bolt' }
  if (route.path === '/strings') return { to: '/guide', label: 'Install Guide', icon: 'i-heroicons-book-open' }
  return { to: '/guide', label: 'Start Setup', icon: 'i-heroicons-arrow-right' }
})
...
const footerGroups = computed(() => [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Contact', href: 'mailto:contact@magguui.com', icon: 'i-heroicons-envelope' },
    ],
  },
])
